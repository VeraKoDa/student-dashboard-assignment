...
function Chart3() {
  ...
  const groups = d3.map(studentData, (d) => d.group).keys();
  const svgRef = useRef();
  ...
  function GroupedBarChart(
    data,
    {
      ...
      xDomain = groups, // array of x-values
  ...
  useEffect(() => {
    ...
    svgRef.current = GroupedBarChart(dataWithoutHeader, {
      x: (d) => d.assignment,
      y: (d) => d.difficulty,
      z: (d) => ratingObjects,
      /*  xDomain: d3.groupSort(objectRating, D => d3.sum(D, d => -D.))
      yFormat: "%", */
      yLabel: "rating",
      height: 500,
